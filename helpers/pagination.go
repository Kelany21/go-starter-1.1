...
type Param struct {
	...
	Filters          []string
	Preload          []string
	PreloadCondition map[string][]string
	ShowSQL          bool
}
...
func Paging(p *Param, result interface{}) *Paginator {
	...
	if len(p.Preload) > 0 {
		db = PreloadD(db, p.Preload,p.PreloadCondition)
	}
...
func Page() int {
	page, _ := strconv.Atoi(_const.Services.GIN.DefaultQuery("page", "1"))
	return page
}

func Limit() int {
	page, _ := strconv.Atoi(_const.Services.GIN.DefaultQuery("limit", os.Getenv("LIMIT")))
	return page
}
