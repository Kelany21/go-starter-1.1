...
func ConvertImageToBase(g *gin.Context, filepath string) (bool, string) {
	...
	b, s := DecodeImage(string(rawData))
	return b, s
}

func MultiDecodeImage(images []string) []string {
	var uploadedImages []string
	for _, imageToUpload := range images {
		valid, imageName := DecodeImage(imageToUpload)
		fmt.Println(valid, imageName)
		if valid {
			if os.Getenv("APP_ENV") == "local" {
				imageName = "http://127.0.0.1:9090/public/images/original/" + imageName
			} else {
				imageName = "http://18.203.101.73/public/images/original/" + imageName
			}
 ...
 */
func DecodeImage(filename string) (bool, string) {
